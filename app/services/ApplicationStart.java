    ...
    private void populateDB() {
        if (Ebean.find(Contact.class).findRowCount() == 0) {
            IntStream.range(0, 100).boxed().forEach(i -> {
                ...
                contact.title = generateName() + " " + generateName();
                contact.save();
            });
//            InputStream is = this.getClass().getClassLoader().getResourceAsStream("initial-data.yml");
//            @SuppressWarnings("unchecked")
//            Map<String, List<Object>> all = (Map<String, List<Object>>)Yaml.load(is, this.getClass().getClassLoader());
//            Ebean.saveAll(all.get("contacts"));
        }
    }
